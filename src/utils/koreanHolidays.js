export const HOLIDAYS_BY_YEAR = {
    ...
    /*
     * 2027 — 음력 공휴일은 확인해서 넣었다(설날 2/7 일요일, 부처님오신날 5/13,
     * 추석 9/15). 나머지 대체공휴일은 `withSubstituteHolidays`가 계산한다.
     *
     * **설날·추석의 대체공휴일만 손으로 넣는다.** 그 둘은 `SUBSTITUTE_ELIGIBLE`에
     * 없다 — 연휴 사흘 중 어느 날이 겹쳤는지에 따라 결과가 달라져서 규칙만으로는
     * 못 맞춘다. 2027년 설날은 일요일(2/7)이라 하루가 밀리는데, 다음 평일인
     * 2/8은 이미 연휴라 2/9(화)가 된다.
     */
    2027: [
