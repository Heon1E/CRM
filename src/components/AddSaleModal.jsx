...
const AddSaleModal = ({ isOpen, onClose }) => {
  ...
  // 디버깅: 모달이 열릴 때 sales 테이블의 실제 컬럼명 확인
  useEffect(() => {
    if (isOpen) {
      const checkSalesTableSchema = async () => {
        try {
          // sales 테이블에서 데이터 1개 가져오기
          ...
          if (error) {
            console.error('sales 테이블 조회 오류:', error)
            console.warn('💡 sales 테이블이 비어있거나 접근 권한이 없을 수 있습니다.')
            console.warn('💡 Supabase 대시보드에서 sales 테이블의 스키마를 직접 확인하거나,')
            console.warn('💡 임시로 데이터를 1개 추가한 후 다시 시도해보세요.')
            return
          }
          ...
          if (data && data.length > 0) {
            ...
            console.log('📊 sales 테이블 실제 컬럼명 확인')
            console.log('='.repeat(60))
            console.log('실제 DB 컬럼명:', columnNames)
            console.log('전체 데이터 샘플:', firstRow)
            console.log('='.repeat(60))
            
            // 주요 컬럼명 확인
            const keyFields = {
              '거래처 ID': ['clientId', 'client_id', 'clientId'],
              '판매일': ['sale_date', 'date', 'saleDate'],
              '품목명': ['item_name', 'itemName', 'product_name'],
              '수량': ['quantity'],
              '단가': ['unit_price', 'unitPrice'],
              '총액': ['total_amount', 'totalAmount', 'supply_price'],
              '비고': ['notes'],
            }
            
            console.log('\n🔍 주요 필드 매핑 확인:')
            Object.entries(keyFields).forEach(([fieldName, possibleKeys]) => {
              const foundKey = possibleKeys.find(key => columnNames.includes(key))
              if (foundKey) {
                console.log(`  ✅ ${fieldName}: "${foundKey}" (값: ${firstRow[foundKey]})`)
              } else {
                console.log(`  ❌ ${fieldName}: 찾을 수 없음 (시도한 키: ${possibleKeys.join(', ')})`)
              }
            })
            console.log('='.repeat(60))
          } else {
            console.warn('⚠️ sales 테이블에 데이터가 없습니다.')
            console.warn('💡 임시로 데이터를 1개 추가한 후 다시 시도해보세요.')
            console.warn('💡 또는 Supabase 대시보드에서 sales 테이블의 스키마를 직접 확인하세요.')
          }
        } catch (err) {
          console.error('sales 테이블 스키마 확인 중 오류:', err)
        }
      }
      
      checkSalesTableSchema()
    }
  }, [isOpen])
