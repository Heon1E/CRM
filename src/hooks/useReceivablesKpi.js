 ...
 *
 * **여기서 읽은 숫자를 점수에 넣지는 않는다.** KPI 채권관리가 세는 것은
 * **대손 · 법적 조치 같은 사고 건수**다(사용자 확인). 대장의 연체 거래처 수
 * (36곳)와는 세는 대상이 다르다 — 눈금이 0건 양호 / 1건 보통 / 2건 미흡인
 * 것도 그래서다. 36을 넣으면 언제나 '미흡'이 된다.
 * 사고 건수는 사람이 KPI 카드에서 직접 넣고, 여기서 읽은 값은 그 옆에
 * **참고**로만 보인다 — 지금 얼마가 밀려 있는지는 알아야 하기 때문이다.
 *
