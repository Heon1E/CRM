...
4) collection_report — 「영업사원 거래처별 매출/수금 실적표」
   ...
   { "clientName": "거래처명", "code": "거래처코드", "phone": "전화번호",
     ...
     "collected": "월:금액,…",
     "balance":   "월:금액,…",
     "totals": "이월합계,매출합계,수금합계,잔액합계" }

   - **값마다 몇 월인지를 붙여 적는다.** 예 (중부산업 수금 줄):
     "1:2684000,2:5368000,3:2684000,4:12760000,5:7876000,6:3322000,8:21478600"
     7월이 빈칸이므로 7은 아예 없다. **빈칸은 빼면 된다** — 달을 함께 적으니
     빠져도 뒤가 밀리지 않는다.
   - **읽은 숫자가 몇 번째 칸에 있는지 세어서 그 달을 적어라.** 칸은 왼쪽부터
     1월, 2월, … 12월이다. 값이 있는 칸만 적되 **그 칸의 달을 틀리지 마라** —
     여기가 이 표에서 가장 자주 틀리는 자리다.
   - 세로로 이월/매출/수금/잔액 네 줄이다. **줄을 헷갈리지 마라** —
     맨 위가 이월, 그 다음이 매출, 수금, 맨 아래가 잔액이다.
   - "totals" 는 **맨 오른쪽 '합 계' 열**의 네 값이다(이월/매출/수금/잔액 순서).
     이 값으로 스스로 검산할 수 있다 — 매출 칸들을 다 더하면 매출 합계와 같아야
     한다. 다르면 어느 칸을 빠뜨렸거나 달을 잘못 적은 것이니 다시 세어라.
   ...
   - **합계 열을 월 값에 섞지 마라.** 월 값은 1~12월 열만, 합계는 "totals" 에.
