...
# Claude AI Agent Configuration (새로 추가)
...
- 프로덕션 환경에서는 Vercel/Netlify 환경변수에 `ANTHROPIC_API_KEY`를 설정하세요
...
- `.env` 파일에 `VITE_ANTHROPIC_API_KEY` 또는 `ANTHROPIC_API_KEY`가 설정되어 있는지 확인
- 개발 서버를 재시작했는지 확인
...
### Vercel 배포 후 작동 안 함
- Vercel 환경변수에 `ANTHROPIC_API_KEY`가 설정되어 있는지 확인
...
- 자세한 내용: [Anthropic Pricing](https://www.anthropic.com/pricing)
